...
def test_optional_enrichment_warnings() -> None:
    service = _service(watchlist_fail=True, momentum_fail=True)
    items, warnings = service._enrich_entities(
        [EntityMention(raw_mention="Apple", ticker="AAPL")],
        ProcessOptions(include_watchlist=True, include_momentum=True),
    )
    assert len(items) == 1
...
def test_required_watchlist_enrichment_raises() -> None:
    ...
    with pytest.raises(DependencyUnavailableError):
        service._enrich_entities(
            [EntityMention(raw_mention="Apple", ticker="AAPL")],
            ProcessOptions(include_watchlist=True, watchlist_required=True, include_momentum=False),
        )
